...
\begin{table}[htbp]
\caption{Observed Go $\leftrightarrow$ C++ SCTP Interoperability}
...
\midrule
Go server $\leftarrow$ C++ client & \texttt{stream=3 ppid=101 payload=cpp-to-go} \\
C++ server $\leftarrow$ Go client & \texttt{stream=4 ppid=202 payload=go-to-cpp} \\
\bottomrule
\end{tabular}
\end{table}
...
Std. dev.: 0.006 s\\
Data source:\\
\texttt{paper/repro/data/}\\
\texttt{interop\_matrix\_runtime\_2026-02-14.csv}
}
}
\caption{Runtime stability of the Go $\leftrightarrow$ C++ SCTP matrix on Linux.}
